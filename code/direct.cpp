    ...
    public:
        ...
        Color3f Li(const Scene* scene, Sampler* sampler, const Ray3f& ray) const {
            ...
            else if (its.mesh->isEmitter())
            {
                ...
                EmitterQueryRecord emiterRecord(its.shFrame.n, inverseRay, its.p);
            ...
            else
            {
                ...
                Color3f matSample = materialBSDF->sample(queryRecord, sampler->next2D());

                EmitterQueryRecord emitterRecord(its.shFrame.n, inverseRay, its.p);
                Color3f emitterSample = scene->sampleEmitter(emitterRecord, *sampler);
                ...
                if (scene->rayIntersect(lightSampleRay, lightIts))
                {
                    if (lightIts.mesh->isEmitter())
                    {
                        ...
                        return matSample * emitterSample;
                    }
                }
            }
